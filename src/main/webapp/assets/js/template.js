...
(function () {
  ...
  inputsetElements.forEach((inputsetElement) => {
    inputsetElement.addEventListener("keyup", () => {
      ...
      if (inputsetText.length > 2000) {
        inputsetElement.value = inputsetText.substring(0, 2000);
        inputsetCount.textContent = inputsetElement.value.length;
      }
    });
  });
  ...
  checksetElements.forEach((checksetElement) => {
    ...
    checksetText.addEventListener("click", () => {
      ...
      }else if (!checkboxInput.disabled) {
        checkboxInput.checked = !checkboxInput.checked;
      }
    });
  });
